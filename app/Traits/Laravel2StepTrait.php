...
trait Laravel2StepTrait  
{
    ...
    public function twoStepVerification($request)
    {
        ...
        if ($user) {
            ...
            if (!$twoStepAuthStatus->authStatus) {
                return false;
            } else {
    ...
    {
		$user = auth()->user();
		if($user->activated == 0) return null;
		$twoStepAuth = $twoStepAuth??$this->getTwoStepAuthStatus($user->id);
    ...
    protected function getTwoStepAuthStatus(int $userId)
    {
        ...
        return TwoStepAuth::where('userId', $userId)->firstOrCreate([
                'userId' => $userId,
            ],
            [
                'userId'    => $userId,
