...
$(document).ready(function() {
	...
	var ajax_post = function( urllink , datam){
		...
		$.post( url , datam, function( data ) {
			...
			unblockUI();
			swal('Process Terminated', errorThrown ,'error' );
		});
					...
					error: function(xhr, status, error) {
						...
						if(typeof data.message !=='undefined'){
							$.each(data.errors, (i,v)=>{
								v = $.isArray(v)?v[0]:v;
								$("#" + i, form).notify(v, {position: 'bottom'});
							});
							return swal('Process Terminated', data.message ,'error');
							...
							return swal('Process Terminated', error,'error' );;
						} else{
							return swal('Process Terminated', error ,'error' );
						}
					}
